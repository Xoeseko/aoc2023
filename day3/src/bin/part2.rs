fn main() {
    ...
    let symbol_positions = _find_asterisk_positions(input);

    let num_with_adjs = numbers.iter().map(move |num| _get_adjacent_positions(*num));



    let res: u32 = _get_number_pairs_with_adjacency(num_with_adjs.collect(), symbol_positions).iter().map(|pair| pair[0] * pair[1]).sum();
    dbg!(res);
}
...
fn _find_asterisk_positions(input_string: &str) -> Vec<Position> {
    input_string
    .lines()
    .enumerate()
    .flat_map(|(line_number, line_content)|
    _find_asterisk_column(line_content, line_number)
)
.collect()
}

fn _find_asterisk_column(line: &str, line_number: usize) -> Vec<Position> {
    line
    .chars()
    .enumerate()
...
fn _get_number_pairs_with_adjacency(numbers: Vec<NumberAdjacencies>, asterisk_positions: Vec<Position>) -> Vec<Vec<u32>> {
    asterisk_positions
    .iter()
    .map(
        |asterik_pos| {
            ...
            .iter()
            .fold(vec![],
                |mut current_list_for_single_asterisk, current_number|
                {
                    if current_number.adjacencies.contains(asterik_pos) {
                        current_list_for_single_asterisk.push(current_number.value);
                    }
                    current_list_for_single_asterisk
                }
            )
        }
    )
    .filter(|potential_gear_list| {
        potential_gear_list.len() == 2
    })
    .collect()
    // numbers
    // .iter()
    // .filter(|num| {
    //     asterisk_positions.iter().any(|symbol_position| num.adjacencies.contains(symbol_position))
    // })
...
.664.598..";
        ...
        let positions = _find_asterisk_positions(input);
    ...
    fn retrieve_symbol_columns_in_signle_line(){
        ...
        let positions = _find_asterisk_column(single_line, 0);

        assert_eq!(positions.len(),1);
        assert_eq!(positions[0], Position{line: 0, column: 5});
...
.664.598..";
    ...
    #[test]
    fn returns_adjacent_number_pairs_by_comparing_list_of_positions_with_list_of_numbers() {
        let input = "467..114..
...*......
..35..633.
......#...
...
.664.598..";
        ...
        let numbers_pairs: Vec<Vec<u32>> = _get_number_pairs_with_adjacency(num_with_adjs.collect(), symbol_positions);
        
        assert!(numbers_pairs.contains(&vec![467u32, 35u32]));
        assert!(numbers_pairs.contains(&vec![755u32, 598u32]));
        assert_eq!(numbers_pairs.len(), 2);
    }
...
.664.598..";
    ...
    let s: u32 = _get_number_pairs_with_adjacency(num_with_adjs.collect(), symbol_positions).iter().map(|pair| pair[0] * pair[1]).sum();
